...
async function staticPlugin({
  ...
  const [fs, path] = builtinModule, fileCache = new LRUCache();
    ...
    if (files.length <= staticLimit)
      for (const absolutePath of files) {
        ...
        if (isBun && absolutePath.endsWith(".html")) {
          const htmlBundle = await import(absolutePath), normalizedPath2 = pathName.replace(/\\/g, "/");
          app.get(normalizedPath2, htmlBundle.default), indexHTML && normalizedPath2.endsWith("/index.html") && app.get(
            normalizedPath2.replace("/index.html", ""),
            htmlBundle.default
          );
          continue;
        }
        extension || (pathName = pathName.slice(0, pathName.lastIndexOf(".")));
        const file = isBun ? getFile(absolutePath) : await getFile(absolutePath);
        if (!file)
          return silent || console.warn(
            `[@elysiajs/static] Failed to load file: ${absolutePath}`
          ), new Elysia();
        const etag = await generateETag(file), normalizedPath = pathName.replace(/\\/g, "/");
        app.get(
          normalizedPath,
        ...
        ), indexHTML && normalizedPath.endsWith("/index.html") && app.get(
  ...
  if (
    ...
    if (isBun) {
      ...
      for (const absolutePath of htmls) {
        if (!absolutePath || shouldIgnore(absolutePath)) continue;
        let relativePath = absolutePath.replace(assetsDir, "");
        const normalizedPath = path.join(prefix, relativePath).replace(/\\/g, "/"), htmlBundle = await import(absolutePath);
        app.get(normalizedPath, htmlBundle.default), indexHTML && normalizedPath.endsWith("/index.html") && app.get(
          normalizedPath.replace("/index.html", ""),
          htmlBundle.default
        );
      }
